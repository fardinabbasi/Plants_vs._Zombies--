...
MelonPult::MelonPult(map<string, float> config,string melon_path,const Vector2f& position,FloatRect bg_bound): 
Plant(config,"MelonPult.png", position), bg_bound(bg_bound)
{
...
void MelonPult::render(RenderWindow &window){
    ...
    for_each(melons.begin(), melons.end(), [&window](pair<Sprite, list<float>> melon) {window.draw(melon.first); });
}